...
#                              Astar_functions.py                              #
#                                                                              #
# Contains the functions for the A* algorithm                                  #
#                                                                              #
# Change History                                                               #
...
from Astar_classes import Node, Edge
import numpy as np
import time
...
#  Function:  A_star
# --------------------
# use the A* algorithm to go from the source node to the target node
# inputs:
#  xlim, ylim: size of the graph
#  x_obs, y_obs: position of the obstacles
...
def A_star(xlim, ylim, w, x_obs, y_obs, start_node_name, target_node_name):
    ...
    while A:
        ...
        for n in v_n_neighbors:
            ...
            gdist = val_n+w
            hdist = np.sqrt(
                (target_node_name[0] - n.name[0]) ** 2 + (target_node_name[1] - n.name[1])**2)
            fdist = gdist+hdist
